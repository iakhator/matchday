...
from app.core.api_keys import ANONYMOUS, ApiKey, find_key, parse_api_keys
...
async def require_api_key(api_key: str = Security(api_key_header)) -> ApiKey:
    ...
    Note this fails *closed*. With no keys configured and no explicit
    opt-out, every request is refused. The previous behaviour was to
    disable auth entirely when the list was empty, which meant a
    ...
    matched = find_key(keys, api_key)
    if not matched:
        raise HTTPException(status_code=401, detail="Invalid or missing X-Gateway-Key")

    # Keyed by name, so rotating a consumer's secret does not hand them a
    # fresh allowance mid-minute.
    retry_after = limiter.check(matched.name, matched.requests_per_minute)
